...
def product_quantity_ordered():
    products_ordered = Product.objects.values('name').annotate(
        total_ordered_products=Sum(
            'orderproduct__quantity'
        )).exclude(total_ordered_products=None).order_by('-total_ordered_products')

    result = []

    for product in products_ordered:
        result.append(f"Quantity ordered of {product['name']}: {product['total_ordered_products']}")
...
def filter_products():
    ...
    filtered_products = [f"{p.name}: {p.price}lv." for p in products]
...
def give_discount():
    ...
    all_products_available = Product.objects.filter(
        is_available=True).order_by('-price', 'name')

    display_projects = [f"{p.name}: {p.price}lv." for p in all_products_available]

    return '\n'.join(display_projects)
